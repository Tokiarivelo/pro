...
namespace ImageExtractapp
{
    ...
    public static class PdfImageExtractor
    {
        ...
        /// <returns>Returns a generic <see cref="MediaTypeNames.Image"/>,
        ...
        public static Dictionary<string, System.Drawing.Image> ExtractImages(string filename, int pageNumber)
        {
            ...
            if (listener.Images.Count > 0)
            {
                MessageBox.Show($"Found {listener.Images.Count} images on page {pageNumber}.");
                foreach (KeyValuePair<System.Drawing.Image, string> pair in listener.Images)
                {
                    images.Add($"{System.IO.Path.GetFileNameWithoutExtension(filename)} _Page_{pageNumber.ToString("D4")} Image_{index.ToString("D4")}  {pair.Value}", pair.Key);
                    index++;
                }
            }
            return images;
